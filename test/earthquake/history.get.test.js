...
describe('GET /earthquakes/history/:country', () => {
	...
	it('Should return all earthquake records for a specific country', async () => {
		await request(app).post('/earthquakes').send(sampleItem1).expect(201);
		await request(app).post('/earthquakes').send(sampleItem2).expect(201);
		await request(app).post('/earthquakes').send(sampleItem3).expect(201);
